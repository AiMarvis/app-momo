...
For the Homebrew cask, the checksum in `AiMarvis/homebrew-momo` must match the exact `Momo.app.tar.gz` asset published on the GitHub release. After replacing that asset, update `Casks/momo.rb` and verify with `brew fetch --cask aimarvis/momo/momo` plus a temp `brew install --cask`.

The web `release.json` should only include updater platform entries when a signed updater artifact has been generated. Unsigned Homebrew-only releases should still update the GitHub download link, but leave updater platforms empty.
