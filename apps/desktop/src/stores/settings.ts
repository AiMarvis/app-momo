...
export type ProjectIssueLanguage = UiLanguage;

interface GeneralSettings {
  autoSave: boolean;
  spellCheck: boolean;
  typingIndicator: boolean;
  projectIssueLanguage: ProjectIssueLanguage;
}
...
interface PersistedSettings {
  ...
  general?: {
    ...
    project_issue_language?: ProjectIssueLanguage;
  };
  appearance?: {
    theme?: ThemePreference;
    language?: UiLanguage;
...
function cachePatchFromObject(value: unknown): SettingsPatch | null {
  ...
  const projectIssueLanguage = asUiLanguage(value.projectIssueLanguage);
  if (projectIssueLanguage) general.projectIssueLanguage = projectIssueLanguage;
...
function patchFromLegacySettings(value: unknown): SettingsPatch | null {
  ...
  if (generalRaw) {
    ...
    const projectIssueLanguage = asUiLanguage(
      generalRaw.projectIssueLanguage ?? generalRaw.project_issue_language,
    );
    if (projectIssueLanguage) general.projectIssueLanguage = projectIssueLanguage;
    if (Object.keys(general).length > 0) patch.general = general;
  }
...
function patchFromPersistedSettings(value: unknown): SettingsPatch | null {
  ...
  if (generalRaw) {
    ...
    if (typingIndicator !== undefined) general.typingIndicator = typingIndicator;
    const projectIssueLanguage = asUiLanguage(generalRaw.project_issue_language);
    if (projectIssueLanguage) general.projectIssueLanguage = projectIssueLanguage;
    if (Object.keys(general).length > 0) patch.general = general;
  }
...
function toPersistedSettings(settings: Settings): PersistedSettings {
  const persisted: PersistedSettings = {
    ...
    general: {
      ...
      project_issue_language: settings.general.projectIssueLanguage,
    },
    appearance: {
...
function cacheFromSettings(settings: Settings): SettingsCache {
  return {
    theme: settings.appearance.theme,
    language: settings.appearance.language,
    projectIssueLanguage: settings.general.projectIssueLanguage,
